---
title: "Age and sex-related variability in the presentation of anxiety and depression symptoms in the GLAD Study"
...
If all three are set to FALSE it runs the script on the whole sample. If you set one of them to TRUE all participants after this date are excluded from analysis
```{r prepandemic indicator}
prepandemic.31.Jan = FALSE
prepandemic.01.Mar = FALSE
prepandemic.23.Mar = TRUE
```

```{r redefining prepandemic sample}
if (prepandemic.31.Jan == TRUE){
  dat <- dat %>%
    filter(prepandemic_january_31 == "Sign up before 31st January")
}

if (prepandemic.01.Mar == TRUE){
  dat <- dat %>%
    filter(prepandemic_march_1 == "Sign up before 1st March")
}

if (prepandemic.23.Mar == TRUE){
  dat <- dat %>%
    filter(prepandemic_march_23 == "Sign up before 23rd March")
}
```
...
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
  ...
  write_rds(
    ...
    file = paste0(saved_r_objects_path,"OR_pub_table_full_sample", date, ".rds")
    )
  
xlsx::write.xlsx(
  x = ALL_CI.ordered.publication,
  file = paste0(saved_r_objects_path,"OR_pub_table_full_sample", date, ".xlsx"),
...
prepandemic.31.Jan = FALSE
prepandemic.01.Mar = FALSE
prepandemic.23.Mar = FALSE

Save 3 different files with correct file extension

```{r Write publication table to excel file prepandemic.31.Jan}
if(prepandemic.31.Jan == TRUE) {
  
  write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_pub_table_prepandemic.31.Jan", date, ".rds")
    )
  
xlsx::write.xlsx(
  x = ALL_CI.ordered.publication,
  file = paste0(saved_r_objects_path,"OR_pub_table_prepandemic.31.Jan", date, ".xlsx"),
  sheetName = "OR_pub_table", row.names = TRUE
  )
}
```

```{r Write publication table to excel file prepandemic.01.Mar}
if(prepandemic.01.Mar == TRUE) {
  
  write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_pub_table_prepandemic.01.Mar", date, ".rds")
    )
  
  
xlsx::write.xlsx(
  x = ALL_CI.ordered.publication,
  file = paste0(saved_r_objects_path,"OR_pub_table_prepandemic.01.Mar", date, ".xlsx"),
  sheetName = "OR_pub_table", row.names = TRUE
  )
}
```

```{r Write publication table to excel file prepandemic.23.Mar}
if(prepandemic.31.Jan == TRUE) {
  
   write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_pub_table_prepandemic.23.Mar", date, ".rds")
    )
  
xlsx::write.xlsx(
  x = ALL_CI.ordered.publication,
  file = paste0(saved_r_objects_path,"OR_pub_table_prepandemic.23.Mar", date, ".xlsx"),
  sheetName = "OR_pub_table", row.names = TRUE
  )
}
```
...
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
 ggsave(
   paste0("combined_symptoms_plot_full_sample", date, ".png"),
...
```{r save as png combined_symptoms_plot prepandemic.31.Jan}
if(prepandemic.31.Jan == TRUE) {
 ggsave(
   paste0("combined_symptoms_plot_prepandemic.31.Jan", date, ".png"),
   plot = combined_symptoms_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png combined_symptoms_plot prepandemic.01.Mar}
if(prepandemic.01.Mar == TRUE) {
 ggsave(
   paste0("combined_symptoms_plot_prepandemic.01.Mar", date, ".png"),
   plot = combined_symptoms_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png combined_symptoms_plot prepandemic.23.Mar}
if(prepandemic.23.Mar == TRUE) {
 ggsave(
   paste0("combined_symptoms_plot_prepandemic.23.Mar", date, ".png"),
   plot = combined_symptoms_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r ALL_CI.age.ordered.cat.publication Format publication table}
ALL_CI.age.ordered.cat.publication <- ALL_CI.age.ordered.cat %>%
  mutate(
    OR_rounded = round(OR, digits = 2),
    OR.CI.lower_rounded = round(OR.CI.lower, digits = 2),
    OR.CI.upper_rounded = round(OR.CI.upper, digits = 2),
...
ALL_CI.age.ordered.cat.publication
```

```{r Write ALL_CI.age.ordered.cat.publication publication table to excel file}
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
  
  write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_age_cat_pub_table_full_sample", date, ".rds")
    )
  
  
xlsx::write.xlsx(
  x = ALL_CI.age.ordered.cat.publication,
  file = paste0(saved_r_objects_path,"OR_age_cat_pub_table_full_sample", date, ".xlsx"),
  sheetName = "OR_pub_table", row.names = TRUE
  )
}
```
...
```{r Write ALL_CI.age.ordered.cat.publication publication table to excel file prepandemic.31.Jan}
if(prepandemic.31.Jan == TRUE) {
  
  write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_age_cat_pub_table_prepandemic.31.Jan", date, ".rds")
    )
  
xlsx::write.xlsx(
  x = ALL_CI.age.ordered.cat.publication,
  file = paste0(saved_r_objects_path,"OR_age_cat_pub_table_prepandemic.31.Jan", date, ".xlsx"),
  sheetName = "OR_age_cat_pub_table", row.names = TRUE
  )
}
```

```{r Write ALL_CI.age.ordered.cat.publication publication table to excel file prepandemic.01.Mar}
if(prepandemic.01.Mar == TRUE) {
  
  write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_age_cat_pub_table_prepandemic.01.Mar", date, ".rds")
    )
  
xlsx::write.xlsx(
  x = ALL_CI.age.ordered.cat.publication,
  file = paste0(saved_r_objects_path,"OR_age_cat_pub_table_prepandemic.01.Mar", date, ".xlsx"),
  sheetName = "OR_age_cat_pub_table", row.names = TRUE
  )
}
```

```{r Write ALL_CI.age.ordered.cat.publication publication table to excel file prepandemic.23.Mar}
if(prepandemic.31.Jan == TRUE) {
  
  write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_age_cat_pub_table_prepandemic.23.Mar", date, ".rds")
    )
  
xlsx::write.xlsx(
  x = ALL_CI.age.ordered.cat.publication,
  file = paste0(saved_r_objects_path,"OR_age_cat_pub_table_prepandemic.23.Mar", date, ".xlsx"),
  sheetName = "OR_age_cat_pub_table", row.names = TRUE
  )
}
```
...
```{r save as png age.cat.plot full_sample}
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
 ggsave(
   paste0("age.cat.plot_full_sample", date, ".png"),
   plot = age.cat.plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png age.cat.plot prepandemic.31.Jan}
if(prepandemic.31.Jan == TRUE) {
 ggsave(
   paste0("age.cat.plot_prepandemic.31.Jan", date, ".png"),
   plot = age.cat.plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png age.cat.plot prepandemic.01.Mar}
if(prepandemic.01.Mar == TRUE) {
 ggsave(
   paste0("age.cat.plot_prepandemic.01.Mar", date, ".png"),
   plot = age.cat.plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png age.cat.plot prepandemic.23.Mar}
if(prepandemic.23.Mar == TRUE) {
 ggsave(
   paste0("age.cat.plot_prepandemic.23.Mar", date, ".png"),
   plot = age.cat.plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
 ggsave(
   paste0("factorsum_correlation_heat_map_full_sample", date, ".png"),
...
```{r save as png factorsum_correlation_heat_map prepandemic.31.Jan}
if(prepandemic.31.Jan == TRUE) {
 ggsave(
   paste0("factorsum_correlation_heat_map_prepandemic.31.Jan", date, ".png"),
   plot = factorsum_correlation_heat_map,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 6.5,
...
```{r save as png factorsum_correlation_heat_map prepandemic.01.Mar}
if(prepandemic.01.Mar == TRUE) {
 ggsave(
   paste0("factorsum_correlation_heat_map_prepandemic.01.Mar", date, ".png"),
   plot = factorsum_correlation_heat_map,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 6.5,
...
```{r save as png factorsum_correlation_heat_map prepandemic.23.Mar}
if(prepandemic.23.Mar == TRUE) {
 ggsave(
   paste0("factorsum_correlation_heat_map_prepandemic.23.Mar", date, ".png"),
   plot = factorsum_correlation_heat_map,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 6.5,
...
ALL_CI_factor.ordered.publication <- ALL_CI_factor.ordered %>%
  mutate(
    estimate_rounded = round(estimate, digits = 4),
    estimate.CI.lower_rounded = round(conf.low, digits = 4),
    estimate.CI.upper_rounded = round(conf.high, digits = 4),
    P_converted = case_when(
      p.value.adjusted < 0.001 ~ formatC(p.value.adjusted, format = "e", digits = 2),
      p.value.adjusted >= 0.001 & p.value.adjusted < 0.01 ~ formatC(p.value.adjusted, format = "f",  digits = 3),
      p.value.adjusted >= 0.01 ~ formatC(p.value.adjusted, format = "f", digits = 2)
...
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
  ...
  write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_ALL_CI_factor.ordered_full_sample", date, ".rds")
    )
  
xlsx::write.xlsx(
  x = ALL_CI_factor.ordered.publication,
  file = paste0(saved_r_objects_path,"OR_ALL_CI_factor.ordered_full_sample", date, ".xlsx"),
...
```{r Write ALL_CI_factor.ordered publication table to excel file prepandemic.31.Jan}

write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_ALL_CI_factor.ordered_prepandemic.31.Jan", date, ".rds")
    )

if(prepandemic.31.Jan == TRUE) {
xlsx::write.xlsx(
  x = ALL_CI_factor.ordered.publication,
  file = paste0(saved_r_objects_path,"OR_ALL_CI_factor.ordered_prepandemic.31.Jan", date, ".xlsx"),
  sheetName = "ALL_CI_factor.ordered", row.names = TRUE
  )
}
```

```{r Write ALL_CI_factor.ordered publication table to excel file prepandemic.01.Mar}

write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_ALL_CI_factor.ordered_prepandemic.01.Mar", date, ".rds")
    )

if(prepandemic.01.Mar == TRUE) {
xlsx::write.xlsx(
  x = ALL_CI_factor.ordered.publication,
  file = paste0(saved_r_objects_path,"OR_ALL_CI_factor.ordered_prepandemic.01.Mar", date, ".xlsx"),
  sheetName = "ALL_CI_factor.ordered", row.names = TRUE
  )
}
```

```{r Write ALL_CI_factor.ordered publication table to excel file prepandemic.23.Mar}

write_rds(
    x = ALL_CI.ordered.publication,
    file = paste0(saved_r_objects_path,"OR_ALL_CI_factor.ordered_prepandemic.23.Mar", date, ".rds")
    )

if(prepandemic.31.Jan == TRUE) {
xlsx::write.xlsx(
  x = ALL_CI_factor.ordered.publication,
  file = paste0(saved_r_objects_path,"OR_ALL_CI_factor.ordered_prepandemic.23.Mar", date, ".xlsx"),
  sheetName = "ALL_CI_factor.ordered", row.names = TRUE
  )
}
```
...
#   device = "png",
#   path = paste0(plots_path, "age_and_sex/factors/"),
#   width = 8,
#   height = 9
# )
```
...
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
 ggsave(
   paste0("combined_factors_plot_full_sample", date, ".png"),
   plot = combined_factors_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png combined_factors_plot prepandemic.31.Jan}
if(prepandemic.31.Jan == TRUE) {
 ggsave(
   paste0("combined_factors_plot_prepandemic.31.Jan", date, ".png"),
   plot = combined_factors_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png combined_factors_plot prepandemic.01.Mar}
if(prepandemic.01.Mar == TRUE) {
 ggsave(
   paste0("combined_factors_plot_prepandemic.01.Mar", date, ".png"),
   plot = combined_factors_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png combined_factors_plot prepandemic.23.Mar}
if(prepandemic.23.Mar == TRUE) {
 ggsave(
   paste0("combined_factors_plot_prepandemic.23.Mar", date, ".png"),
   plot = combined_factors_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 9,
...
```{r save as png combined_factors_plot_without_sum full_sample}
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
 ggsave(
   paste0("combined_factors_plot_without_sum_full_sample", date, ".png"),
...
```{r save as png combined_factors_plot_without_sum prepandemic.31.Jan}
if(prepandemic.31.Jan == TRUE) {
 ggsave(
   paste0("combined_factors_plot_without_sum_prepandemic.31.Jan", date, ".png"),
   plot = combined_factors_plot_without_sum,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 6,
...
```{r save as png combined_factors_plot_without_sum prepandemic.01.Mar}
if(prepandemic.01.Mar == TRUE) {
 ggsave(
   paste0("combined_factors_plot_without_sum_prepandemic.01.Mar", date, ".png"),
   plot = combined_factors_plot_without_sum,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 6,
...
```{r save as png combined_factors_plot_without_sum prepandemic.23.Mar}
if(prepandemic.23.Mar == TRUE) {
 ggsave(
   paste0("combined_factors_plot_without_sum_prepandemic.23.Mar", date, ".png"),
   plot = combined_factors_plot_without_sum,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 6,
...
if(prepandemic.31.Jan == FALSE | prepandemic.01.Mar == FALSE | prepandemic.23.Mar == FALSE) {
 ggsave(
   paste0("age_sex_symptom_factor_plot_full_sample", date, ".png"),
...
```{r save as png age_sex_symptom_factor_plot prepandemic.31.Jan}
if(prepandemic.31.Jan == TRUE) {
 ggsave(
   paste0("age_sex_symptom_factor_plot_prepandemic.31.Jan", date, ".png"),
   plot = age_sex_symptom_factor_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 11,
...
```{r save as png age_sex_symptom_factor_plot prepandemic.01.Mar}
if(prepandemic.01.Mar == TRUE) {
 ggsave(
   paste0("age_sex_symptom_factor_plot_prepandemic.01.Mar", date, ".png"),
   plot = age_sex_symptom_factor_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 11,
...
```{r save as png age_sex_symptom_factor_plot prepandemic.23.Mar}
if(prepandemic.23.Mar == TRUE) {
 ggsave(
   paste0("age_sex_symptom_factor_plot_prepandemic.23.Mar", date, ".png"),
   plot = age_sex_symptom_factor_plot,
   device = "png",
   path = paste0(plots_path, "age_and_sex/symptoms"),
   width = 11,
